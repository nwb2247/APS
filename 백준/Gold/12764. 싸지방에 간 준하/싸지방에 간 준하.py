...
=> 리팩토링 : 매 시간(최대 1_000_000)마다 돌 필요 없이, 내가 시작하려는 시점에 종료되었어야 하는 사람들 다 종료 시켜버리면 됨
...
for sh, eh in lst:
    ...
    while ongoing and ongoing[0][0] <= sh:          # 지금 시간까지 종료했어야 하는 사람들 다 빼줌
    ...
    heappush(ongoing, (eh, com_to_use))     # ongoing에 (종료시간, 컴퓨터 번호)를 넣어줌
