...
[2차 풀이]
- 정리했었던 당기기 로직을 사용
- 행방향 열방향 헷갈리거나 사방으로 기울이고, 당기고 하는 것들이 있다면 회전으로도 구현가능한지 생각해보자
- 구현을 마치고 주석을 꼼꼼히 달고 확인하면서 로직상 문제가 있는지, 오타가 있는지 점검가능
...
왼쪽 당기는 방법만 생각하고
배열을 왼쪽으로 90도씩 돌려가면서 확인

5번 이동하여 얻을 수 있는 최대 블록의 크기

이동할때마다 작아지는 경우는 없으므로 일단 5회 돌리고 max를 찾아보자
(최대 블록 크기를 출력하는 것이므로 돌려도 상관 x)
백트래킹

4**5 -> 가능, 깊이도 깊지 않으므로 배열 복사 해도됨
"""
...
def pull(arr):
    ...
    for cr in range(N):
        tmp = []
        for cc in range(N):
            if arr[cr][cc] != 0:   # 0아닌 것들 일단 tmp에 담아둠
                tmp.append(arr[cr][cc])

        merged = []
        ci = 0
        while ci<len(tmp):
            if ci+1<len(tmp) and tmp[ci] == tmp[ci+1]: # ci+1가 oob 아니고, ci와 일치한다면
                merged.append(tmp[ci]*2)        # 합쳐서 넣어주고, 다다음 것으로 넘어감 (다음 것은 이미 합쳐졌으므로)
                ci += 2
            else:   # 다음 것이 oob거나 다음것과 동일하지 않다면,,, 내거만 넣어주고 다음것으로 넘어감
                merged.append(tmp[ci])
                ci += 1

        for cc in range(N):
            if cc < len(merged): # cc가 merged의 인덱스와 일치하는 경우만 왼쪽부터 채워서 넣어줌
                narr[cr][cc] = merged[cc]
            else:
                narr[cr][cc] = 0
...
def dfs(depth, arr):
    ...
    """
    :param arr: 도착한 배열
    :param depth: 지금까지 몇번 움직였는지
    :return:
    """
    if depth == 5: # 이미 5번 다 움직였다면
        mx = max(map(max, arr))
        ans = max(ans, mx)
        return

    # pull, arr 모두 새 배열 생성
    dfs(depth + 1, pull(arr)) # 그냥 당겨서 호출
    for _ in range(3): # 한번씩 더 돌려서 호출 1, 2, 3번까지 돌림 (4번째는 360도 이므로 원래와 동일)
        arr = rotate(arr)
        dfs(depth + 1, pull(arr))
