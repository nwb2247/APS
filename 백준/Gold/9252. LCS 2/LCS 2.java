...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		int[][][] prev = new int[input1.length+1][input2.length+1][];
			...
			for (int j=1; j<=input2.length; j++) {
				if (input1[i-1] != input2[j-1]) {
					if (DP[i-1][j] > DP[i][j-1]) {
						DP[i][j] = DP[i-1][j];
						prev[i][j] = new int[] {i-1, j};
					} else {
						DP[i][j] = DP[i][j-1];
						prev[i][j] = new int[] {i,j-1};
					}
				} else {
					DP[i][j] = DP[i-1][j-1]+1;
					prev[i][j] = new int[] {i-1,j-1};
				}
			}
		}
		...
		int[] cur = new int[] {input1.length, input2.length};
		while (cur[0] != 0 && cur[1] != 0) {
			if (input1[cur[0]-1] == input2[cur[1]-1]) {
				stack.addLast(input1[cur[0]-1]);
			}
