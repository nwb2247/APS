"""
[2차 풀이]
1차 풀이와 달리 len으로 두개가 이어지는지 확인후 True, False 판단

[이해 및 구상]
인접한 두 계란 양 차이가 L이상 R이면 연결
...
def check():
    changed = False
    v = [[0 for _ in range(N)] for _ in range(N)]  # 한 틀은 한그룹에만 들어감
    for sr in range(N):
        for sc in range(N):
            if v[sr][sc] != 0:  # 방문한 곳이면 안들어감
                continue
            v[sr][sc] = 1
            q = deque()
            q.append((sr, sc))
            ...
            while q:
                ...
                for dr, dc in ds:
                    nr, nc = cr + dr, cc + dc
                    if oob(nr, nc) or not L <= abs(m[cr][cc] - m[nr][nc]) <= R or v[nr][nc] != 0:
                        continue
                    v[nr][nc] = 1
                    q.append((nr, nc))
            ...
            if len(group) > 1:
                changed = True
                sm = sum(map(lambda x: m[x[0]][x[1]], group))
                for cr, cc in group:
                    m[cr][cc] = sm // len(group)

    return changed  # 바뀌는 것이 없다면 False 반환


def solve():
    cnt = 0
    while check(): # 바뀌었다면 cnt += 1해줌 (바뀌지 않을때까지 계속)
        cnt += 1
        continue
    print(cnt)
    return
