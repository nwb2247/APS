...
# 크루스칼을 이용한 최소신장트리 + 유니온파인드 (경로압축, 유니온바이랭크까지 적용)
...
def union(x, y):  # rank가 더 작은 것을 rank가 더 큰 것의 자식으로 만들어줌 (그래야 좀 더 높이가 작아짐)
    x_r = find(x)
    y_r = find(y)
    if x_r == y_r:              # 이미 같은 집합이면 return
        return
    elif rank[x]>rank[y]:       # x_r 의 랭크가 y_r의 랭크보다 더 크다면
        parent[y_r] = x_r       # y_r을 x_r에 붙임
    elif rank[y]>rank[x]:       # y_r 의 랭크가 x_r의 랭크보다 더 크다면
        parent[x_r] = y_r       #
    else: # rank[x_r] == rank[y_r]    # rank가 같다면
        parent[y_r] = x_r             # 한쪽에 붙여주고
        rank[x_r] += 1                # root 된 쪽에 += 1
...
rank = [1]*(V+1)                    # rank : 높이의 근사치 (경로 압축과 같이 쓰기 때문에 실제 높이와 차이가 있을 수 있음)
